...
class relicAPI:
    ...
    def getMatches(self, in_steam_id, n_matches):
        ...
        resp_matches = requests.get(
            "https://aoe-api.reliclink.com/community/leaderboard/getRecentMatchHistory?title=age2&profile_names=[%22%2Fsteam%2F"
            + str(in_steam_id)
            + "%22]"
        ).json()
        ...
        try:
            ...
            for matches_history in resp_matches.get("matchHistoryStats"):
                ...
                title_contains_string = (
                    (search_string == title)
                    or (search_string.capitalize() == title)
                    or (search_string.upper() == title)
                    or (search_string + " " in title)
                    or (" " + search_string in title)
                    or (search_string.capitalize() + " " in title)
                    or (" " + search_string.capitalize() in title)
                    or (search_string.upper() + " " in title)
                    or (" " + search_string.upper() in title)
                )
                if not (
                    title_contains_string
                ):  # or matches_history.get("description").contains("DITO") or matches_history.get("description").contains("Dito"))
                    continue
                else:
                    steam_id.clear()
                    names.clear()
    ...
    def getElos(self, in_steam_id):
        try:
            resp_profile = requests.get(
                "https://aoe-api.reliclink.com/community/leaderboard/GetPersonalStat?title=age2&profile_names=[%22%2Fsteam%2F"
                + str(in_steam_id)
                + "%22]"
            ).json()
    ...
    def findLobby_byID(self, lobby_id):
        ...
        steam_id = []
        try:
            resp_lobby = requests.get(
                "https://aoe-api.reliclink.com/community/advertisement/findAdvertisements?title=age2"
            ).json()
